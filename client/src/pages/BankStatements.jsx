...
export default function BankStatements() {
  ...
  const saveTransaction = async (changes) => {
    if (!statement || !transaction) return;
    try {
  ...
  const approve = async () => {
    ...
    try {
      ...
      setStatements((current) => current.map((entry) => entry._id === updated._id ? updated : entry));
      setNotice('Statement approved and locked.');
      setError('');
    } catch (err) {
      setError(err.message);
    }
    ...
    <>
      ...
      <section className="statement-review">
        <div className="statement-toolbar">
          ...
          {loading ? (
            <div className="statement-loading" aria-busy="true"><Skeleton label="Loading statements…" /><Skeleton /><div className="card statement-table-card"><table><TableSkeleton columns={5} label="Loading statement transactions…" /></table></div></div>
          ) : !statement ? (
            ...
            <>
              <div className="card statement-summary">
                ...
                <div className="statement-summary-actions">
                  {statement.status === 'approved' ? <span className="approved-lock">Locked</span> : <><span className={unresolved ? 'review-count' : 'ready-count'}>{unresolved ? `${unresolved} need review` : 'Ready to approve'}</span>{can('statements.approve') && <button type="button" disabled={unresolved > 0} onClick={approve}>Approve statement</button>}</>}
                </div>
              </div>
